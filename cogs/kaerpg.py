    ...
    async def battlecontroller(self, ctx, player: dict, dungeon: str):
        ...
        for enemyindex in range(1, KaeRPG.dungeons[dungeon]["Number of Enemies"] + 1):
            ...
            while True:
                ...
                action = await self.bot.wait_for("message", check=lambda m: m.author == ctx.author and m.channel == ctx.channel and m.content.lower() in actions)
        ...
        async with self.bot.kaedb.acquire() as conn:
            async with conn.transaction():
                if await conn.fetchrow("SELECT * FROM kaerpg_characterinfo WHERE user_id = $1", str(ctx.author.id)):
                    ...
                    while True:
                        name = await self.bot.wait_for("message", check=lambda m: m.author == ctx.author and m.channel == ctx.channel)
                        name = name.content
                        if len(name) <= 20:
                            break
                        else:
                    ...
                    while True:
                        statchoice = await self.bot.wait_for("message", check=lambda m: m.author == ctx.author and m.channel == ctx.channel)
                    ...
                    embed.clear_fields()
                    while True:
                        weapon = await self.bot.wait_for("message", check=lambda m: m.author == ctx.author and m.channel == ctx.channel)
                    ...
                    embed.clear_fields()
                    while True:
                        armour = await self.bot.wait_for("message", check=lambda m: m.author == ctx.author and m.channel == ctx.channel)
        ...
        async with self.bot.kaedb.acquire() as conn:
            async with conn.transaction():
                if await conn.fetch("SELECT * FROM kaerpg_characterinfo WHERE user_id = $1", str(ctx.author.id)):
                    ...
                    while True:
                        check = await self.bot.wait_for("message", check=lambda m: m.author == ctx.author and m.channel == ctx.channel)
