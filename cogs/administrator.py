...
class Administrator:
    ...
    @commands.group(
        name="prefix",
        brief="Server-specific prefix commands. Run to view prefixes.",
    ...
    @commands.command(
        name="prune",
        brief="Prunes members from the server.",
    ...
    async def prune(self, ctx, daysinactive: int = 5):
        if ctx.author.guild_permissions.administrator:
            ...
            if not estimate == 0:
                await ctx.send(
                    f"\U00002757This opeation will prune **{estimate}** from the server who have been inactive"
                    f" for the past **{daysinactive}** days and who have no roles. Proceed? (y\\n)"
                )
            else:
                return await ctx.send(
                    "No eligible, prunable members of this server who have been inactive for the past"
                    f" {daysinactive} days."
                )
